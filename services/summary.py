...
from services.loaders import site_master_dict, valid_sites
import pandas as pd
...
def build_summary(filepath):
    ...
    for site_code in valid_sites:
        ...
        if site_code in env_info:
            row["ENV Alarms"] = " | ".join([escape_but_allow_br(a) for a in env_info[site_code]["alarms"]])
    ...
    return df, dashboard, dashboard_summary, tables_down_env, tables_env_only, tech_labels, tech_counts, down_type_counts, env_labels, env_values, excel_path
